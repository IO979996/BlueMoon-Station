...
/datum/antagonist/ninja/proc/addMemories()
	...
	antag_memory += "У меня есть личный корабль (додзё) в космосе с крио-стазисом: после выполнения целей могу вернуться на корабль и уйти в крио, чтобы покинуть раунд.<br>"

...
/proc/melee_blood_stain_attacker(mob/living/carbon/human/attacker, mob/living/victim, damage_done)
	...
	if(damage_done >= 20 || (damage_done >= 15 && prob(25)))
		attacker.get_item_by_slot(ITEM_SLOT_ICLOTHING)?.add_mob_blood(victim)
		attacker.get_item_by_slot(ITEM_SLOT_OCLOTHING)?.add_mob_blood(victim)
	if(prob(33) && damage_done >= 10)
		attacker.get_item_by_slot(ITEM_SLOT_FEET)?.add_mob_blood(victim)
	if(prob(33) && damage_done >= 24)
		attacker.get_item_by_slot(ITEM_SLOT_MASK)?.add_mob_blood(victim)
	if(prob(33) && damage_done >= 30)
		attacker.get_item_by_slot(ITEM_SLOT_HEAD)?.add_mob_blood(victim)

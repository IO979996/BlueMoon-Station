 ...
 */
/datum/component/shell/proc/on_set_anchored(atom/movable/source, new_anchored)
	SIGNAL_HANDLER
...
/datum/component/shell/proc/on_item_interaction(atom/source, mob/living/user, obj/item/item, list/modifiers)
	...
	if(istype(item, /obj/item/inducer))
		...
		if(attached_circuit)
			inducer.recharge(attached_circuit, user)
		else
			inducer.recharge(parent, user)
...
/datum/component/shell/proc/override_power_usage(datum/source, power_to_use)
	...
	if(power_used_in_minute > max_power_use_in_minute)
		...
		if(attached_circuit)
			remove_circuit()
		return
	// BlueMoon: no per-tile APC terminal API; track draw for overheat only.
